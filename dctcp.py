...
def dctcp():
    ...
    for i in xrange(args.hosts):
        ...
        net.getNodeByName(node_name).popen('/bin/ping -c 100 -i 0.1 10.0.0.1 >> %s/k%d_h%d_ping.txt' % (args.dir, args.mark_threshold, i), shell=True)
        net.getNodeByName(node_name).popen('awk -F '/' 'END {print $5}' %s/k%d_h%d_ping.txt >> %s/k%d_h%d_ping_avg.txt' % (args.dir, args.mark_threshold, i), shell=True)
